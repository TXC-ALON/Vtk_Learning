...
#define SIZEOF_EDGE_SORT 24
  ...
  typedef struct stl_vertex
  {
    float x;
    float y;
    float z;
    void stl_scale(float factor)
    {
      x /= factor;
      y /= factor;
      z /= factor;
    }
    bool operator==(const stl_vertex &rt) const
    {
  ...
  typedef char stl_extra;
  typedef int stl_face_side;
  typedef int stl_face_type;
  typedef int stl_face_type_id;

  struct stl_point_in_facet
  {
    stl_normal normal;
    stl_vertex onepoint;
    int facet_idx;

    bool operator<(const stl_point_in_facet &rt)
    {
      return this->onepoint.z < rt.onepoint.z;
    }

    double operator-(const stl_point_in_facet &rt)
    {
      return this->onepoint.z - rt.onepoint.z;
    }

    stl_point_in_facet operator=(const stl_point_in_facet &rt)
    {
      this->facet_idx = rt.facet_idx;
      this->onepoint = rt.onepoint;
      this->normal = rt.normal;
      return *this;
    }
  };
  ...
  struct stl_facet
  {
    stl_normal normal;
    stl_vertex vertex[3];
    stl_extra extra;               // 该字段暂未被使用
    stl_face_side face_side;       // 支架上下表面标记
    stl_face_type face_type;       // 支架的各部件标记
    stl_face_type_id face_type_id; // 支架的各部件id
    ...
    bool concide_face(const stl_facet &rt)
    {
      for (int i = 0; i < 3; i++)
      {
        for (int j = 0; j < 3; j++)
        {
          if (rt.vertex[i] == this->vertex[j])
            return true;
        }
      }
      return false;
...
#define SIZEOF_STL_FACET 50
  ...
  typedef struct
  {
    ...
    int shared_malloced;
    bool unable_repair;
    bool no_need_fix_normal_direction;
  } stl_stats;

  typedef struct
  {
    int inverted_normals;       // 反向三角面片
    int bad_edges;              // 坏边
    int bad_contours;           // 错误轮廓
    int near_bad_edges;         // 缝隙
    int planar_holes;           // 空洞
    int noise_shells;           // 干扰壳体
    int overlapping_triangles;  // 重叠三角面片
    int intersecting_triangles; // 交叉三角片片
    int non_manifold;           // 非流形错误
  } stl_repair_stats;
  ...
  typedef struct
  {
    ...
    v_indices_struct *v_indices; // facet_id--->verts_id
    stl_vertex *v_shared;
    stl_stats stats;
    char error;
    stl_repair_stats repair_stats;
  } stl_file;

  extern void stl_open(stl_file *stl, const char *file);
  extern void stl_wopen(stl_file *stl, const wchar_t *file);
  ...
  extern void stl_write_binary_w(stl_file *stl, const wchar_t *file, const char *label);
  ...
  extern void stl_fix_normal_directions(stl_file *stl, bool auto_repair = true);
  extern void stl_fix_normal_values(stl_file *stl, bool auto_repair = true);
  ...
  extern void stl_fix_facets_shared_one_edge(stl_file *stl, bool auto_repair);

  extern void stl_repair_init(stl_file *stl);

  extern float stl_get_area(stl_facet *facet);

  extern float get_volume(stl_file *stl);
  extern float get_area(stl_file *stl);
  extern void stl_remove_degenerate(stl_file *stl, int facet_number);
