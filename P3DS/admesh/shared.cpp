...
stl_invalidate_shared_vertices(stl_file *stl) {
  ...
  if (stl->error) return;
  if (stl->stats.unable_repair) return;
...
void stl_generate_shared_vertices(stl_file *stl) 
{
	...
	if (stl->v_indices == NULL)
	{
		LOGINFO("stl->v_indices calloc failed");
		return;
	}
	...
	if (stl->v_shared == NULL)
	{
		LOGINFO("stl->v_shared calloc failed");
		return;
	}
	...
	for(i = 0; i < stl->stats.number_of_facets; i++) 
	{
		...
		{
			// 
			if(stl->v_indices[i].vertex[j] != -1) continue;
			// 存储空间不足  再申请
			...
			while(true)
			{
				...
				//LOGINFO("facet_num[%d], pivot_vertex[%d], first_facet[%d], next_facet[%d],next_edge[%d], vnot[%d]",
				//	facet_num,
				//	pivot_vertex,
				//	first_facet,
				//	next_facet,
