...
void
stl_wopen(stl_file *stl, const wchar_t *file) {
	stl_initialize(stl);
	stl_count_facets_w(stl, file);
	stl_allocate(stl);
	stl_read(stl, 0, 1);
	fclose(stl->fp);
}
...
stl_initialize(stl_file *stl) {
  ...
  stl->stats.shared_malloced = 0;

  stl->stats.unable_repair = false;
  stl->stats.no_need_fix_normal_direction = false;
...
stl_count_facets(stl_file *stl, const char *file) {
  ...
  LOGINFO("Find size of file [%ld]", file_size);
  /* Check for binary or ASCII file */
  fseek(stl->fp, HEADER_SIZE, SEEK_SET);
  if (!fread(chtest, sizeof(chtest), 1, stl->fp)) {
	  LOGINFO("The input is an empty file");
      stl->error = 1;
      return;
  }
  ...
  rewind(stl->fp);
  LOGINFO("Check for binary or ASCII file");
...
void
stl_count_facets_w(stl_file *stl, const wchar_t *file) {
	long           file_size;
	int            header_num_facets;
	int            num_facets;
	int            i;
	...
	stl->fp = _wfopen(file, L"rb");
	if (stl->fp == NULL) {
		LOGINFO("stl_initialize: Couldn't open for reading _wfopen");
	...
	if (stl->stats.type == binary) {
		...
		if (((file_size - HEADER_SIZE) % SIZEOF_STL_FACET != 0)
			|| (file_size < STL_MIN_FILE_SIZE)) {
			LOGINFO("The file has the wrong size");
			OutputDebugStringW(file);
			stl->error = 1;
			return;
		}
	...
	/* Otherwise, if the .STL file is ASCII, then do the following */
		...
		// fix to silence a warning about unused return value.
		// obviously if it fails we have problems....
		stl->fp = _wfreopen(file, L"r", stl->fp);

		// do another null check to be safe
		if (stl->fp == NULL) {
			LOGINFO("stl_initialize: Couldn't open for reading _wfreopen");
			OutputDebugStringW(file);
			stl->error = 1;
			return;
		}
