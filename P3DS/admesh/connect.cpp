...
static bool stl_is_two_facet_almost_coplane(stl_facet facet_1, stl_facet facet_2, float tolerance)
{
	if (fabs(facet_1.normal.x - facet_2.normal.x) < tolerance
		&&fabs(facet_1.normal.y - facet_2.normal.y) < tolerance
		&&fabs(facet_1.normal.z - facet_2.normal.z) < tolerance)
	{
		return true;
	}
	else if (fabs(fabs(facet_1.normal.x) - fabs(facet_2.normal.x)) < tolerance
		&&fabs(fabs(facet_1.normal.y) - fabs(facet_2.normal.y)) < tolerance
		&&fabs(fabs(facet_1.normal.z) - fabs(facet_2.normal.z)) < tolerance)
	{
		return true;
	}
	else
...
void stl_fix_facets_shared_one_edge(stl_file *stl, bool auto_repair = true)
{
	...
	if (auto_repair)
	{
		LOGINFO("repairing... 开始进行共边拓扑错误修复。");
	}
	else
	{
		LOGINFO("checking... 开始进行共边拓扑错误检查。");
	}
	...
	if (checked_facet_ids == NULL) LOGINFO("stl_fix_normal_directions checked_facet_ids");

	normal_error_loop = (stl_edge*)calloc(stl->stats.number_of_facets, sizeof(stl_edge));
	if (normal_error_loop == NULL) LOGINFO("stl_fix_normal_directions normal_error_loop");

	many_shared_facet_ids = (int*)calloc(stl->stats.number_of_facets, sizeof(int));
	if (many_shared_facet_ids == NULL) LOGINFO("stl_fix_normal_directions many_shared_facet_ids");

	all_many_shared_facet_ids = (int*)calloc(stl->stats.number_of_facets, sizeof(int));
	if (all_many_shared_facet_ids == NULL) LOGINFO("stl_fix_normal_directions all_many_shared_facet_ids");

	has_deleted_facet_ids = (int*)calloc(stl->stats.number_of_facets, sizeof(int));
	if (has_deleted_facet_ids == NULL) LOGINFO("stl_fix_normal_directions has_deleted_facet_ids");
	...
	for (int i = 0; i < stl->stats.number_of_facets; i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			{				
				...
				LOGINFO("法向错误边数 %d 大于面片总数 %d，标记不可修复为 %d。", normal_error_count, stl->stats.number_of_facets, stl->stats.unable_repair);
				return;
			}
		}
	}
	for (int i = 0; i < normal_error_count; i++)
	{
		...
		if (count == 4)
		{
			...
			{
				LOGINFO("checking... 拿到一组共边面片，本组共边面片数量 %d。存在任意两个面片面积相等。", count);//跳过异常支撑的同时，跳过了相同错误的模型。
				for (int n = 0; n < count; n++)
				{
			...
			{
				//拿到一组共边面片
				LOGINFO("repairing... 拿到一组共边面片，本组共边面片数量 %d。", count);
			}
			else
			{
				//拿到一组共边面片
				LOGINFO("checking... 拿到一组共边面片，本组共边面片数量 %d。", count);
			}
			...
			for (int n = 0; n < count; n++)
			{
				...
				if (auto_repair)
				{
					LOGINFO("repairing... 本组共边面片中第 %d 个面片 ID 为 %d，面积为 %f。", n, facet_num, get_area(&temp_facet_n));
				}
				else
				{
					LOGINFO("checking... 本组共边面片中第 %d 个面片 ID 为 %d，面积为 %f。", n, facet_num, get_area(&temp_facet_n));
				}
				int tmp = 0;
				int tmp_1 = 0;
				...
				if (auto_repair)
				{
					LOGINFO("repairing... 本组共边面片中共发现 %d 个面片与自身共面，共边面片总数 %d，细小退化面片数量 %d。", tmp, count, tmp_1);
				}
				else
				{
					LOGINFO("checking... 本组共边面片中共发现 %d 个面片与自身共面，共边面片总数 %d，细小退化面片数量 %d。", tmp, count, tmp_1);
				}
				if (tmp == count)
				{
					for (int l = 0; l < count; l++)
					{
						if (removed_count != 0)
						{
							...
							else
							{
								...
								if (auto_repair)
								{
									LOGINFO("repairing... 多面片共边同时共面，共 %d 个面片 ，将要删除第 %d 个面片，facet_num %d", count, l, many_shared_facet_ids[l]);
								}
								else
								{
									LOGINFO("checking... 多面片共边同时共面，共 %d 个面片 ，将要删除第 %d 个面片，facet_num %d", count, l, many_shared_facet_ids[l]);
								}
							}
						}
						else
				...
				else if (tmp_1 == count)
				{
					for (int l = 0; l < count; l++)
					{
						...
						if (auto_repair)
						{
							LOGINFO("repairing... 多面片共边同时都非常小，共 %d 个面片 ，删除第 %d 个面片，facet_num %d", count, l, many_shared_facet_ids[l]);
						}
						else
						{
							LOGINFO("checking... 多面片共边同时都非常小，共 %d 个面片 ，删除第 %d 个面片，facet_num %d", count, l, many_shared_facet_ids[l]);
						}
					}
					break;
				}
				else
				{
					if (auto_repair)
					{
						LOGINFO("repairing... 发现共边异常拓扑结构，将强行相互标记法向判断无效。面片自身 ID %d。", facet_num);
					}
					else
					{
						LOGINFO("checking... 发现共边异常拓扑结构，将强行相互标记法向判断无效。面片自身 ID %d。", facet_num);
					}
					for (int j = 0; j < 3; j++)
					{
						if (stl->neighbors_start[facet_num].which_vertex_not[j] > 2)
						{
							...
							if (auto_repair)
							{
								LOGINFO("repairing... 主面片 ID %d，主面片面积 %f，发现邻居 %d，邻居面片 ID %d，标记相互法向判断无效。", facet_num, get_area(&temp_facet), j, stl->neighbors_start[facet_num].neighbor[j]);
								stl->neighbors_start[facet_num].is_invalid[j] = true;
							}
							else
							{
								LOGINFO("checking... 主面片 ID %d，主面片面积 %f，发现邻居 %d，邻居面片 ID %d，标记相互法向判断无效。", facet_num, get_area(&temp_facet), j, stl->neighbors_start[facet_num].neighbor[j]);
							}
						}
					}
				}
			...
			{
				LOGINFO("repairing... 邻居匹配成功。一组共边面片中 %d 与 %d 是邻居，%d 与 %d 是邻居。", facet_num_a1, facet_num_a2, facet_num_b1, facet_num_b2);
			...
			{
				LOGINFO("checking... 邻居匹配失败。失败结果一组共边面片中 %d 与 %d 不是邻居，%d 与 %d 不是邻居。", facet_num_a1, facet_num_a2, facet_num_b1, facet_num_b2);
			}
		}
		else if (count > 4) 
		{
			...
			{
				//拿到一组共边面片
				LOGINFO("repairing... 拿到一组共边面片，本组共边面片数量 %d。将直接删除。", count);
			}
			else
			{
				//拿到一组共边面片
				LOGINFO("checking... 拿到一组共边面片，本组共边面片数量 %d。将直接删除。", count);
			}
			for (int n = 0; n < count; n++)
			{
		...
		else 
		{
			if (count != 2)
			{
				LOGINFO("发现共边面片数量 %d。不做处理。仅标记相互法向判断无效。", count);
				for (int n = 0; n < count; n++)
				{
					int facet_num = many_shared_facet_ids[n];
					for (int j = 0; j < 3; j++)
	...
	if (normal_error_count != 0)
	{
		...
		{
			LOGINFO("repairing... 法向错误边数量与共边拓扑异常边数量相同，标记无需进行法向修复为 true。");
			stl->stats.no_need_fix_normal_direction = true;
		}
	...
	if ((removed_count != 0) && (stl->stats.no_need_fix_normal_direction == false) && auto_repair)
	{
		...
		for (int i = 0; i < removed_count; i++)
		{
			...
			LOGINFO("repairing... 此修复方法结束，一次性删除需要删除的面片。删除第 %d 个面片 %d，共 %d 个。", i, all_many_shared_facet_ids[i], removed_count);
			has_deleted_facet_ids[all_many_shared_facet_ids[i]] = 1;
		}
		LOGINFO("repairing... 删除面片后自定义修复。");
		stl_repair(stl, 0, 0, 1, 1, 1, 1, 1, 2, 0, 0, 0, 0, 0, 0);
	}	
	if (normal_error_count && auto_repair) LOGINFO("repairing... 法向相反的边的个数 %d。", normal_error_count);
	if (more_two_facets_shared_one_edge_count && auto_repair) LOGINFO("repairing... 共边拓扑异常边个数 %d。", more_two_facets_shared_one_edge_count);
...
static bool stl_swap_facets_neighbor(stl_file *stl, int facet_num_a1, int facet_num_a2, int facet_num_b1, int facet_num_b2) 
{
	LOGINFO("repairing... 开始进行邻居交换，拓扑重建。");
	float tolerance = 0.000001;//过滤掉面积为 0 的异常面片
	if (stl_is_degenerate_tiny_facet(stl, facet_num_a1, tolerance) == true)
	{
		LOGINFO("repairing... 发现太过细小干扰面片，本组邻居交换取消！细小面片 %d，阈值 %f 。", facet_num_a1, tolerance);
		return false;
	}
	if (stl_is_degenerate_tiny_facet(stl, facet_num_a2, tolerance) == true)
	{
		LOGINFO("repairing... 发现太过细小干扰面片，本组邻居交换取消！细小面片 %d，阈值 %f 。", facet_num_a2, tolerance);
		return false;
	}
	if (stl_is_degenerate_tiny_facet(stl, facet_num_b1, tolerance) == true)
	{
		LOGINFO("repairing... 发现太过细小干扰面片，本组邻居交换取消！细小面片 %d，阈值 %f 。", facet_num_b1, tolerance);
		return false;
	}
	if (stl_is_degenerate_tiny_facet(stl, facet_num_b2, tolerance) == true)
	{
		LOGINFO("repairing... 发现太过细小干扰面片，本组邻居交换取消！细小面片 %d，阈值 %f 。", facet_num_b2, tolerance);
		return false;
	}
	...
	for (int i = 0; i < 3; i++)
	{
		if (stl->neighbors_start[facet_num_a1].which_vertex_not[i] > 2) 
		{
			if (facet_num_a1_n == -1 && facet_num_a1_w == -1 && facet_num_a1_i == -1)
			{
				...
				LOGINFO("repairing... 面片 %d 邻居交换完成！", facet_num_a1);
			}
			else
			{
				time++;
		...
		if (stl->neighbors_start[facet_num_a2].which_vertex_not[i] > 2)
		{
			if (facet_num_a2_n == -1 && facet_num_a2_w == -1 && facet_num_a2_i == -1)
			{
				...
				LOGINFO("repairing... 面片 %d 邻居交换完成！", facet_num_a2);
			}
			else
			{
				time++;
		...
		if (stl->neighbors_start[facet_num_b1].which_vertex_not[i] > 2)
		{
			if (facet_num_b1_n == -1 && facet_num_b1_w == -1 && facet_num_b1_i == -1)
			{
				...
				LOGINFO("repairing... 面片 %d 邻居交换完成！", facet_num_b1);
			}
			else
			{
				time++;
		...
		if (stl->neighbors_start[facet_num_b2].which_vertex_not[i] > 2)
		{
			if (facet_num_b2_n == -1 && facet_num_b2_w == -1 && facet_num_b2_i == -1)
			{
				...
				LOGINFO("repairing... 面片 %d 邻居交换完成！", facet_num_b2);
			}
			else
			{
				time++;
...
stl_check_facets_exact(stl_file *stl) {
  ...
  */
	LOGINFO("checking or repairing... 开始精确建立面片拓扑关系。");
  stl_hash_edge  edge;
  stl_facet      facet;
  int            i;
  int            j;
...
stl_check_facets_nearby(stl_file *stl, float tolerance) {
	LOGINFO("repairing... 开始近似建立面片拓扑关系。");
