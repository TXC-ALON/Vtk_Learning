...
bool stl_is_two_point_equal(stl_vertex point_1, stl_vertex point_2)
{
	if ((fabs(point_1.x - point_2.x) < 0.000001) && (fabs(point_1.y - point_2.y) < 0.000001) && (fabs(point_1.z - point_2.z) < 0.000001))
	{
		return true;
	}
	else
...
stl_reverse_facet(stl_file *stl, int facet_num)
{
	...
	if (neighbor[0] != -1)
		stl->neighbors_start[neighbor[0]].which_vertex_not[(vnot[0] + 1) % 3] =
			(stl->neighbors_start[neighbor[0]].which_vertex_not[(vnot[0] + 1) % 3] + 3) % 6;
	if (neighbor[1] != -1)
		stl->neighbors_start[neighbor[1]].which_vertex_not[(vnot[1] + 1) % 3] =
			(stl->neighbors_start[neighbor[1]].which_vertex_not[(vnot[1] + 1) % 3] + 4) % 6;
	if (neighbor[2] != -1)
		stl->neighbors_start[neighbor[2]].which_vertex_not[(vnot[2] + 1) % 3] =
			(stl->neighbors_start[neighbor[2]].which_vertex_not[(vnot[2] + 1) % 3] + 2) % 6;
...
void stl_fix_normal_directions(stl_file *stl, bool auto_repair)
{
	if (auto_repair)
	{
	}
	...
	if (head == NULL)
		LOGINFO("stl_fix_normal_directions");
...
int stl_check_normal_vector(stl_file *stl, int facet_num, int normal_fix_flag)
{
	...
	if ((ABS(normal[0] - facet->normal.x) < 0.001) && (ABS(normal[1] - facet->normal.y) < 0.001) && (ABS(normal[2] - facet->normal.z) < 0.001))
	{
	...
	if ((ABS(normal[0] - test_norm[0]) < 0.001) && (ABS(normal[1] - test_norm[1]) < 0.001) && (ABS(normal[2] - test_norm[2]) < 0.001))
	{
		if (normal_fix_flag)
		{
			facet->normal.x = normal[0];
...
void stl_fix_normal_values(stl_file *stl, bool auto_repair)
{
