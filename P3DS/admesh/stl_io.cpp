...
stl_write_binary(stl_file *stl, const char *file, const char *label) {
  ...
  fprintf(fp, "%s", label);
  for(i = strlen(label); i < LABEL_SIZE; i++) 
	  putc(0, fp);
...
void
stl_write_binary_w(stl_file *stl, const wchar_t* file, const char* label) {
	FILE      *fp;
	int       i;
	//char      *error_msg;
	...
	if (fp == NULL) {
		LOGINFO("stl_write_binary_w: Couldn't open for reading _wfopen");
...
stl_write_neighbor(stl_file *stl, int facet) {
  if (stl->error) return;
  LOGINFO("Neighbors %d: %d, %d, %d ;  %d, %d, %d\n", facet,
