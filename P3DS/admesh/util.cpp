...
stl_verify_neighbors(stl_file* stl) {
	...
	for (i = 0; i < stl->stats.number_of_facets; i++) {
		for (j = 0; j < 3; j++) {
			...
			if (neighbor == -1) {
				LOGINFO("/* this edge has no neighbor... Continue. */");
				continue;
			}
			if (vnot < 3) {
			...
			if (memcmp(&edge_a, &edge_b, SIZEOF_EDGE_SORT) != 0) {
				...
				LOGINFO("repaired... 警告。edge %d of facet %d doesn't match edge %d of facet %d.", j, i, vnot + 1, neighbor);
...
stl_get_size(stl_file* stl) {
	...
	stl->stats.min.x = FLT_MAX;
	stl->stats.min.y = FLT_MAX;
	stl->stats.min.z = FLT_MAX;
	stl->stats.max.x = -FLT_MAX;
	stl->stats.max.y = -FLT_MAX;
	stl->stats.max.z = -FLT_MAX;
	...
	{
		LOGINFO("stl->stats.number_of_facets == 0");
		return;
	}
	...
	//LOGINFO("End stl_get_size  min [%f,%f,%f] max [%f,%f,%f]",
	   // stl->stats.min.x,
	   // stl->stats.min.y,
	   // stl->stats.min.z,
	   // stl->stats.max.x,
...
float stl_get_area(stl_facet* facet)
{
	return get_area(facet);
}
...
void stl_repair(stl_file* stl,
	int fixall_flag,  //是否修复所有  
	int exact_flag,   //是否进行精确检查            
	int tolerance_flag, //是否要求在公差范围内
	float tolerance,  //公差范围值
	int increment_flag, //是否要求增量
	float increment,  //增量值
	int nearby_flag,  //是否进行相邻检查  
	int iterations, //迭代循环次数
	int remove_unconnected_flag,  //是否移除所有孤立面片
	int fill_holes_flag,  //是否填补空洞
	int normal_directions_flag, //是否修复面片法向向量方向错误
	int normal_values_flag,	//是否修复面片法向向量值错误
	int reverse_all_flag, //是否翻转所有面片
	int verbose_flag) {//是否记录日志
	LOGINFO("repairing... 开始调用综合修复方法。");
	int i;
		...
		if (stl->stats.connected_facets_3_edge < stl->stats.number_of_facets) {
			for (i = 0; i < iterations; i++) {
				if (stl->stats.connected_facets_3_edge <
					stl->stats.number_of_facets) {
					if (verbose_flag)
						LOGINFO("Checking nearby. Tolerance= %f Iteration=%d of %d...", tolerance, i + 1, iterations);
					stl_check_facets_nearby(stl, tolerance);
					if (verbose_flag)
						LOGINFO("  Fixed %d edges.\n", stl->stats.edges_fixed - last_edges_fixed);
					last_edges_fixed = stl->stats.edges_fixed;
					tolerance += increment;
				}
				else {
					if (verbose_flag)
						LOGINFO("All facets connected.  No further nearby check necessary.\n");
					break;
				}
			}
		}
			...
			if (verbose_flag)
				LOGINFO("All facets connected.  No nearby check necessary.\n");
		}
	}
	...
	if (remove_unconnected_flag || fixall_flag || fill_holes_flag) {
		if (stl->stats.connected_facets_3_edge < stl->stats.number_of_facets) {
			if (verbose_flag)
				LOGINFO("Removing unconnected facets...\n");
			stl_remove_unconnected_facets(stl);
		}
		else
			if (verbose_flag)
				LOGINFO("No unconnected need to be removed.\n");
	}

	if (fill_holes_flag || fixall_flag) {
		if (stl->stats.connected_facets_3_edge < stl->stats.number_of_facets) {
			if (verbose_flag)
				LOGINFO("Filling holes...\n");
			stl_fill_holes(stl);
		}
		else
			if (verbose_flag)
				LOGINFO("No holes need to be filled.\n");
	}

	if (reverse_all_flag) {
		if (verbose_flag)
			LOGINFO("Reversing all facets...\n");
		stl_reverse_all_facets(stl);
	}

	if (normal_directions_flag || fixall_flag) {
		if (verbose_flag)
			LOGINFO("Checking normal directions...\n");
		stl_fix_normal_directions(stl);
	}

	if (normal_values_flag || fixall_flag) {
		if (verbose_flag)
			LOGINFO("Checking normal values...\n");
		stl_fix_normal_values(stl);
	}

	/* Always calculate the volume.  It shouldn't take too long */
	if (verbose_flag)
		LOGINFO("Calculating volume...\n");
	stl_calculate_volume(stl);

	if (exact_flag) {
		if (verbose_flag)
			LOGINFO("Verifying neighbors...\n");
